...
export const CommitsGrid = ({ text }: { text: string }) => {
  ...
  const [animationTrigger, setAnimationTrigger] = React.useState(0); // State to trigger animation replay

  const getRandomColor = () => {
    const commitColors = [
      "#1d4ed8", // Light vibrant blue
      "#3b82f6", // Medium vibrant blue
      "#1d4ed8", // Dark vibrant blue
    ];
  ...
  const handleMouseEnter = () => {
    setAnimationTrigger((prevTrigger) => prevTrigger + 1); // Increment trigger to re-run animations
  };

  return (
    <section
      className="w-full max-w-xl bg-card  grid p- sm:p-1 gap-0.9 sm:gap-0.9 rounded-[10px] sm:rounded-[15px] mx-auto"
      // w-full: Sets width to 100%.
      // max-w-xl: Sets maximum width to extra large.
      // bg-card: Sets background color to card color (theme-dependent).
      // grid: Applies CSS Grid layout.
      // p-1 sm:p-2: Sets padding (small on mobile, larger on small screens and up). // Note: current is p- sm:p-1
      // gap-px sm:gap-0.5: Sets gap between grid items (pixel on mobile, larger on small screens and up). // Note: current is gap-0.9 sm:gap-0.9
      // rounded-[10px] sm:rounded-[15px]: Sets border radius (10px on mobile, 15px on small screens and up).
      // mx-auto: Centers the element horizontally.
      onMouseEnter={handleMouseEnter} // Add mouse enter event handler
      style={{
        gridTemplateColumns: `repeat(${gridWidth}, minmax(0, 1fr))`, // Defines grid columns: repeats gridWidth times, each column takes a minimum of 0 and a maximum of 1 fraction of available space.
        gridTemplateRows: `repeat(${gridHeight}, minmax(0, 1fr))`, // Defines grid rows: repeats gridHeight times, each row takes a minimum of 0 and a maximum of 1 fraction of available space.
      }}
    >
      {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
        ...
        return (
          <div
            key={`${index}-${animationTrigger}`} // Update key to include animationTrigger for re-animation
            className={cn(
              ` h-full w-full aspect-square rounded-[2px] sm:rounded-[4px]`, // Added back 'border' for consistent cell outline
              // border: Adds a border to the element.
              // h-full: Sets height to 100%.
              // w-full: Sets width to 100%.
              // aspect-square: Maintains a 1:1 aspect ratio.
              // rounded-[2px] sm:rounded-[4px]: Sets border radius (2px on mobile, 4px on small screens and up).
              isHighlighted ? "animate-highlight" : "", // Applies highlight animation if isHighlighted is true.
              // shouldFlash ? "animate-flash" : "", // Applies flash animation if shouldFlash is true.
              !isHighlighted && !shouldFlash ? "bg-card/20 border-opacity-10" : "" // If not highlighted and not flashing: sets background to card color with 20% opacity and border opacity to 10%.
            )}
            style={
              {
                animationDelay: getRandomDelay(), // Sets a random delay for the animation.
                "--highlight": getRandomColor(), // Sets a CSS custom property for highlight color.
              } as CSSProperties
            }
          />
        );
